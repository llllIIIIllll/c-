...
double closestPair(vector<point> &pointsByX, vector<point> &pointsByY, ll low, ll high)
{
	...
	// find minumun distance among left half of points recursicely
	...
	// store points in strip of widtg 2d stored by Y coordinates
	vector<point> pointsInStrip;
	for (i = 0; i < n; ++i)
	{
		if (abs(pointsByY[i].x - pointsByY[mid].x) < bestDistance)
				pointsInStrip.push_back(pointsByY[i]);
	}

	// calculate minimun distance within the strip (atmost 6 comparisions)
